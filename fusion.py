# We adopt the implementation of tsdf fusion from https://github.com/andyzeng/tsdf-fusion-python
...
class TSDFVolume:
  ...
  def write_volume(self,vol_cpu):
    if self.gpu_mode:
      cuda.memcpy_htod(self._tsdf_vol_gpu,vol_cpu)
    else:
      self._tsdf_vol_cpu=vol_cpu

  def write_color(self,vol_cpu):
    if self.gpu_mode:
      cuda.memcpy_htod(self._color_vol_gpu,vol_cpu)
    else:
      self._color_vol_cpu=vol_cpu

  def set_white(self):
    if self.gpu_mode:
      white= np.ones(self._vol_dim)*(256**3-1)
      white=white.astype(np.float32)
      cuda.memcpy_htod(self._color_vol_gpu,white)
    else:
      self._color_vol_cpu = np.ones(self._vol_dim,dtype=np.float32)*(256**3-1)
